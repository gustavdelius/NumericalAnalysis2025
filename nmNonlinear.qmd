...
::: {#exr-2.11}
We want to write a Python function for the Bisection Method. Instead of jumping straight into writing the code we should first come up with the structure of the code. It is often helpful to outline the structure as comments in your file. Use the template below and complete the comments.
Don't write the code yet, just the comments. I recommend switching off the AI for this exercise because otherwise the AI will keep already suggesting the code while you write the comments.
...
:::{#exr-bisection-feedback}
Modify your code for the bisection method so that instead of splitting the interval $[a,b]$ at the algebraic mean $a+b/2$ on each iteration, it splits the interval at the geometric mean $m=\sqrt{ab}$. Use a starting interval of $[1,3]$  to find an approximation for the root of $f(x)=\log(x)-\sin(x)$. Stop the method when $|f(m)| < 10^{-5}$. What is $m$ at that point?
:::
...
::: {#exr-2.19}
🎓 Create a second version of your Python Bisection Method function that uses a `for` loop that takes the required number of steps to approximate the root and guarantee that it lies within a requested tolerance. This should be in contrast to your first version which likely used a `while` loop to decide when to stop. Is there an advantage to using one of these version of the Bisection Method over the other?
...
The quadratic convergence implies that if we plot the error in the new iterate on the $y$-axis and the error in the old iterate on the $x$ axis of a log-log plot then we will see a constant slope of 2. To see this we can take the log of both sides of the previous equation to get 
\begin{equation}
